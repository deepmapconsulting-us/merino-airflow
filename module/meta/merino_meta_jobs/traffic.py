...
def campaign_region_daily_snapshot(
    access_token: str,
    account_id: str,
    campaign_ids: list[str],
    metric_date: str,
    *,
    ...
    """Fetch campaign-level daily insights with region breakdown."""
    account_id = ensure_act_prefix(account_id)
    if not campaign_ids:
        return _empty_daily_snapshot(account_id, metric_date)
    ...
    insights = client.get_all(f"{account_id}/insights", params)
    return _daily_snapshot(account_id, metric_date, insights)


def adset_region_daily_snapshot(
    access_token: str,
    account_id: str,
    campaign_id: str,
    adset_ids: list[str],
    ...
    *,
    ...
    """Fetch adset-level daily insights with region breakdown."""
    account_id = ensure_act_prefix(account_id)
    if not adset_ids:
        return _empty_daily_snapshot(account_id, metric_date, campaign_id=campaign_id)
...
def ad_region_daily_snapshot(
    ...
    *,
    ...
    """Fetch ad-level daily insights with region breakdown."""
    account_id = ensure_act_prefix(account_id)
    if not ad_ids:
        return _empty_daily_snapshot(account_id, metric_date, campaign_id=campaign_id)
